...
- Two capability tiers:
  ...
  - **Bridge** (SIP disabled + `imsg launch` dylib injection): `send-rich`, `send-multipart`, `send-attachment`, `sticker`, `tapback`, `poll`, `edit`, `unsend`, `delete-message`, `read`, `typing`, `notify-anyways`, `chat-*`, and default-mode `account`/`whois`/`nickname`.
...
imsg sticker --chat GUID --file ~/Pictures/sticker.png --attach-to MSG_GUID
