...
- `since_rowid` (int, optional) — exclusive cursor; resume from the last message's `cursor` field, not its `id`. Repeated cursors can replay an event, so deduplicate notifications by stable message identity.
